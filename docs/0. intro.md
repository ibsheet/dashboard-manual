...
```html
<!-- IBsheet8, IBChart, IBMap  관련 라이브러리 -->
...
DashBuilder.create({ id: "dash1", el: document.getElementById("dash1"), info: DashboardInfo });
```

### 초기 화면

처음 접속하면 빈 화면이 표시되며, 우측 상단에 버튼 아이콘이 표시됩니다.

![초기 화면](./assets/img/create-widget/widget_general0.png)

### 위젯 추가하기

1. 화면 우측 상단의 아이콘을 클릭합니다.
2. 우측에서 서랍 형태의 사이드바 메뉴가 열립니다.
3. 사이드바 메뉴에는 추가 가능한 위젯 목록이 표시됩니다.
...
- **KPI**: 수치 지표 표시
- **이미지**: 이미지 표시
- **HTML**: HTML 코드를 통한 시각화
- **iframe**: ifame 을 통한 시각화
...
1. 위젯에 마우스를 올리면 오른쪽에 아이콘 버튼이 표시됩니다.
2. 톱니바퀴 아이콘을 클릭하면 우측에 해당 위젯에 대한 설정이 열립니다.
3. 사이드바 메뉴에서 위젯에 데이터를 설정할 수 있습니다.
4. 휴지통 아이콘 버튼을 통해 위젯을 제거할 수 있습니다.
...
![대시보드 예시1](./assets/img/example1.png)

![대시보드 예시2](./assets/img/example2.png)
